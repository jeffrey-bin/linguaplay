...
import type { ChildState, GameInstruction, RecentInstruction } from "@/types/game";
...
function parseAgentResponse(text: string): GameInstruction[] {
  ...
  // Validate and normalize each instruction
  ...
  for (const instr of instructions) {
    ...
    instr.target_items = (instr.target_items as string[]).filter((item) => {
    ...
    // Normalize expected_action
    if (!instr.expected_action || typeof instr.expected_action !== "object") {
      const target = instr.target_items[0] || "apple";
      instr.expected_action = { type: "tap", target, source: null };
    }
    if (instr.expected_action.source === "scene" || instr.expected_action.source === undefined) {
      instr.expected_action.source = null;
    }
    if (!instr.expected_action.type) {
      instr.expected_action.type = instr.interaction_type === "drag" ? "drag" : "tap";
    }

    // Normalize hint_strategy to expected structure
    const target = instr.target_items[0] || instr.expected_action?.target || "apple";
    const wordLabel = target.replace(/_/g, " ");

    if (!instr.hint_strategy || typeof instr.hint_strategy !== "object") {
      instr.hint_strategy = {};
    }

    // Normalize level_1
    if (!instr.hint_strategy.level_1 || typeof instr.hint_strategy.level_1 !== "object") {
      instr.hint_strategy.level_1 = { type: "glow", target };
    }
    if (!instr.hint_strategy.level_1.target) {
      instr.hint_strategy.level_1.target = target;
    }

    // Normalize level_2
    if (!instr.hint_strategy.level_2 || typeof instr.hint_strategy.level_2 !== "object") {
      instr.hint_strategy.level_2 = {
        type: "arrow",
        target,
        audio: `The ${wordLabel}!`,
      };
    }
    if (!instr.hint_strategy.level_2.target) {
      instr.hint_strategy.level_2.target = target;
    }
    if (!instr.hint_strategy.level_2.audio) {
      instr.hint_strategy.level_2.audio = `The ${wordLabel}!`;
    }

    // Normalize level_3
    if (!instr.hint_strategy.level_3 || typeof instr.hint_strategy.level_3 !== "object") {
      instr.hint_strategy.level_3 = {
        type: "show_and_tell",
        fallback_text: `Tap the ${wordLabel}!`,
        highlight_all_targets: true,
      };
    }
    if (!instr.hint_strategy.level_3.fallback_text) {
      instr.hint_strategy.level_3.fallback_text = `Tap the ${wordLabel}!`;
    }
    if (instr.hint_strategy.level_3.highlight_all_targets === undefined) {
      instr.hint_strategy.level_3.highlight_all_targets = true;
    }

    // Ensure defaults for optional fields
    instr.new_word = instr.new_word || null;
    instr.known_words_used = instr.known_words_used || [];
    instr.interaction_type = instr.interaction_type || "tap";
    instr.animation = instr.animation || null;
    instr.success_feedback = instr.success_feedback || "Great job!";
    instr.difficulty_score = instr.difficulty_score ?? 0.3;
  }

  // Filter out instructions with no valid target items
  const validInstructions = instructions.filter(
    (i: any) => i.target_items.length > 0
  );

  if (validInstructions.length === 0) {
    throw new Error("No valid instructions after filtering");
  }

  return validInstructions as GameInstruction[];
}
